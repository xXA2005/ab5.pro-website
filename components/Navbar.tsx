    ...
    {/*  */}
      <div className='max-w-[1440px] h-full mx-auto py-1 flex items-center justify-between '>
        <motion.div initial={{opacity:0}} animate={{opacity:1}} transition={{duration:0.5}}>
          <Image src='/favicon.ico' alt="ab5" width={100} height={100} className='object-contain'/>
        </motion.div>
        <div className='hidden md:inline-flex items-center gap-7'>
          <ul className='flex text-[17px] gap-7'>
            <Link onClick={handleScroll} href="#home" className='flex items-center gap-1 text-blue-500 hover:text-blue-700 cursor-pointer duration-300 nav-link font-medium'><motion.li initial={{y: -10, opacity:0}} animate={{y:0, opacity:1}} transition={{duration:0.1}}>Home</motion.li></Link> 
            <Link onClick={handleScroll} href="#xp" className='flex items-center gap-1 text-blue-500 hover:text-blue-700 cursor-pointer duration-300 nav-link font-medium'><motion.li initial={{y: -10, opacity:0}} animate={{y:0, opacity:1}} transition={{duration:0.2}}>Experince</motion.li></Link> 
        ...
        {/* small icon section */}
        <div onClick={() => setShowMenu(true)} className='w-6 h-5 flex-col justify-between text-4xl text-blue-500 cursor-pointer overflow-hidden md:hidden flex items-center group'>
        ...
        </div>
        {
          showMenu && (
            ...
            className='absolute md:hidden top-0 right-0 w-full h-screen bg-black bg-opacity-50 flex flex-col items-end'>
              <motion.div initial={{opacity:0,x:20}} animate={{x:0,opacity:1}} transition={{duration:0.1}}
              className='w-[80%] h-full overflow-y-scroll bg-gray-800 flex flex-col items-center px-4 py-10 relative'>
                <MdOutlineClose onClick={() => setShowMenu(false)} className='text-3xl text-blue-500 cursor-pointer hover:text-red-500 absolute top-4 right-4'/>
                <div className='flex flex-col items-center gap-7'>
                  <ul className='flex text-[17px] gap-7 flex-col'>
                    <Link onClick={handleScroll} href="#home" className='flex items-center gap-1 text-blue-500 hover:text-blue-700 cursor-pointer duration-300 nav-link font-medium'><motion.li initial={{y: 20, opacity:0}} animate={{y:0, opacity:1}} transition={{duration:0.1,delay:0.1, ease:'easeIn'}}>Home</motion.li></Link> 
                    <Link onClick={handleScroll} href="#xp" className='flex items-center gap-1 text-blue-500 hover:text-blue-700 cursor-pointer duration-300 nav-link font-medium'><motion.li initial={{y: 20, opacity:0}} animate={{y:0, opacity:1}} transition={{duration:0.2,delay:0.1, ease:'easeIn'}}>Experince</motion.li></Link> 
